...
static bool stats_phase_critical(double ms) {
    return ms >= STATS_PHASE_WARN_MS;
}

static bool stats_compile_critical(double ms) {
    return ms >= STATS_COMPILE_WARN_MS;
}

static bool stats_total_critical(double ms) {
    return ms >= STATS_TOTAL_WARN_MS;
}
...
void nine_stats_print(void) {
    ...
    bool banner_critical = preprocess_critical || lex_critical || parse_critical || compile_critical || total_critical;
